...
app.post('/payment', function (request, response) {
    console.log("Start backend payment service B");
    var idParts = request.body.account.split('*');
    var friendlyId = idParts[0];

    client.query('SELECT balance from users where friendlyid = $1', [friendlyId], (error, results) => {
        if (error) {
            console.log("banckend payment service B queery error",error);
            response.json({
                msg: "ERROR!",
                error_msg: error
            });
        ...
        if (results) {
            console.log("backend payment service B queery results",results);
            balance = results.rows[0].balance;
            ...
            requestObj.post({
                ...
                console.log("result of requestObj by backend payment A to bridge server B at port 8007", err, res, body);
