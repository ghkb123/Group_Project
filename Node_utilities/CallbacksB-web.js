...
const app = express();
const pg = require('pg');
const conString = "postgres://bankbuser:bankbuser@localhost:5433/bankb";
...
app.post('/compliance/sanctions', function (request, response) {
	console.log("sanctions request", request.body.sender);
	var sender = JSON.parse(request.body.sender)
	console.log("santions request sender", sender);

	client.query('SELECT * from sanction where domain = $1', [sender.domain], (error, results) => {
		if (error) {
			console.log("santion request error message", error);
			response.status(403).end("FI not sanctioned");
		}
		if (results) {
			//console.log("sanction request query result", result);
			response.status(200).end();
		}
	})
	console.log("santions request -- the end");
});
...
app.post('/compliance/ask_user', function (request, response) {
	...
	client.query('SELECT * from sanction where domain = $1', [sender.domain], (error, results) => {
		...
		// if (results) {
		// 	console.log("ask user query result", results);
		// 	if (results.rows[0].kyc == true) {
		// 		response.status(200).end();
		// 		console.log("ask user request approved");
		// 	}
		// 	else {
		// 		response.status(403).end("KYC request denied");
		// 		console.log("ask user request: KYC request denied");
		// 	}
		// }
		response.status(200).end();
...
app.post('/test', function (request, response) {
	fetch('http://bankb.com/.well-known/stellar.toml')
